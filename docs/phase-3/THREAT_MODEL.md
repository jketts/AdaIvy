...
| Retrieval projections | FTS tables, scores, optional embeddings | rebuildable, no warrant authority |
...
| Malicious PDF/parser exploit | Active content or malformed objects compromise the host | Size/type limits, no script execution, isolated no-network parser process, bounded CPU/memory/time/output, minimal environment | Malformed fixture fails or is quarantined with retained process evidence |
...
| URL/metadata SSRF | A supplied URI causes network access to local/internal resources | Phase 3A metadata import never dereferences; any future fetch adapter needs explicit scheme/host policy and separate ADR | Network-disabled test records zero fetches |
...
| Decompression/parser bomb | Tiny input expands without bound | Input, object, page, time, memory, and output caps | Resource-limit fixture terminates safely |
...
| Embedding drift | Provider/model update silently changes memory | Embeddings are optional projections with provider/model/version/input hash; canonical memory is independent | Dropping embedding index leaves canonical export unchanged |
...
- PDF text order and equation extraction may remain imperfect even when
  deterministic. Warnings and the authoritative original reduce, but do not
  eliminate, expert review.
