...
/* Phase 3B entry-gate v4 launcher: Linux/AArch64 only. */
...
static int allowed_fixture(const char *arg) {
    static const char *const allowed[] = {
        "/trusted/F01.lean", "/trusted/F02.lean", "/trusted/F03.lean",
        "/trusted/F04.lean", "/trusted/F05.lean", "/trusted/F06.lean",
        "/trusted/F07.lean", "/trusted/F08.lean", "/trusted/F09.lean",
        "/trusted/F10.lean", "/trusted/F11.lean", "/trusted/F12.lean"
    };
    size_t i;
    for (i = 0; i < sizeof(allowed) / sizeof(allowed[0]); ++i)
        if (strcmp(arg, allowed[i]) == 0)
            return 1;
    return 0;
}
...
static int memfd_errno(void) {
    int result = (int)syscall(SYS_MEMFD_CREATE, "adaivy-v4-probe", 0);
    if (result >= 0) {
        close(result);
        return 0;
    }
...
static int policy_self_test(int abi) {
    ...
    int pass = (sh != 0 && bash != 0 && env != 0 && launcher != 0 &&
                ...
                memfd == EPERM);
    printf("{\"schema_version\":\"adaivy.landlock-policy-probe.v1\","
           "\"landlock_abi\":%d,\"passed\":%s,"
           ...
           abi, pass ? "true" : "false", sh, bash, env, launcher, loader,
           tmp_copy_status, tmp_copy, shebang_status, shebang, at, memfd);
    return pass ? 0 : 1;
}
...
int main(int argc, char **argv) {
    ...
    if (argc == 2 && strcmp(argv[1], "--policy-self-test") == 0)
        return policy_self_test(abi);
    if (argc != 2 || !allowed_fixture(argv[1])) {
        puts("untrusted or unknown wrapper path rejected");
        return 64;
    }
