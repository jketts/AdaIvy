"""Exact small-matrix algebra over the represented algebraic field.
...
matrices rather than an eigenvalue approximation, and the dimension bound keeps
...
def parse_matrix(value: Any) -> Matrix:
    ...
    rows = tuple(tuple(parse_algebraic(item) for item in row) for row in value if isinstance(row, list))
    ...
    if len(rows) > MAX_DIMENSION:
        raise AlgebraicFieldError("matrix exceeds exact spike dimension bound")
    return rows
...
def total(values: Any) -> AlgebraicComplex:
    result = ZERO
    for value in values:
        result = result + value
    return result
